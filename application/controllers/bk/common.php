...
class Common extends CI_Controller {
  ...
  function generateTable($table_array)
  ...
  function getPatternTableType2BK($table_array) {
	//echo "<pre>" . print_r($table_array, TRUE) . "</pre>";
	  //die();
     $col_1 = 3;
     $col = 2;
 	 $col_title = array(0 => "Top",1 => "Middle", 2 => "Bottom");  	 
 	 $col_left_title = array(0 => "",1 => "T",2 => "M", 3 => "B");  	 
  	 ...
  	 foreach ($table_array as $index => $table_row_data) {
		...
		$table = $this->com->ActiveDocument->Tables->Add($range,3,4,1,2);
		...
		foreach($array_data as $end_key => $table_inner_row_data) {
		
       $table_row = $end_key + 1;
		echo implode("|", $table_inner_row_data)."<br>";
    	 foreach ($table_inner_row_data as $key => $value) {
        $header_col = $key + 1;
        $value = empty($value) ? "  " : $value;
        $table->Cell($table_row,$header_col)->Range->InsertAfter($value);
        $table->Cell($table_row,$header_col)->Range->ParagraphFormat->Alignment = 2;
		
        
		if($index == 1 && $table_row == 1 && $header_col > 1)  {
		...
		else if(($index == 1 && $header_col == 1) || ($index > 1 && ($header_col == 1 || $header_col == 2))){
			$table->Cell($table_row,$header_col)->Borders(-1)->LineStyle  = FALSE;		
...
foreach($final_a_1_row_12 as $kkk => $vvv) {
	 ...
	 echo "ROW :: $row";
  	 ...
  	 foreach ($final_array as $index => $table_row_data) {
		...
		elseif($row == $table_row) {
			...
			if(!$leftBool) {
				echo "LEFT :: $table_row | $header_col :: $value<br>";

								
				$table->Cell($table_row,$header_col)->Borders(-2)->LineStyle  = FALSE;
			}
			if(!$rightBool) {
								echo "RIGHT :: $table_row | $header_col :: $value<br>";
  ...
  function getPatternTableType2YYYYY($table_array) {
	//echo "<pre>" . print_r($table_array, TRUE) . "</pre>";
	  //die();
     $col_1 = 3;
     $col = 2;
	...
	$prange = $this->documents->Tables[1]->Range;
	$ptable = $this->documents->Tables->Add($prange,6,3,1,2);
       ...
       $table_row = $end_key + 1;
		...
		//$this->documents->Tables->Add($table,);
     }
  }
